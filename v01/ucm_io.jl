...
function trans(S::AbstractVector{<:AbstractStanza}; ucb::UniCodebase=UNICODEBASE, suppress_error_nothing_to_show::Bool=false, save_codebase_mode::Symbol=:no)  # :no :copy :OVERWRITE
    ...
    ##### ERRORS
    if transouts === nothing  # check this first, as invalid exitcode might be suppressed from existing transouts
        display(tr);  erroruser("transcript output not found; check output above")
    end

    if result.exitcode != 0
        fail = true
        if suppress_error_nothing_to_show
            n = transouts |> fl(x->occursin("nothing to show", x)) |> length
            n > 0  &&  ( fail = false )
        end
        if fail
    ...
    if save_codebase_mode in (:copy, :OVERWRITE)  &&  suniloc_new === nothing
        display(tr);  erroruser("new codebase not found; check output above")
    end
    #####
...
#trans(x::AbstractStanzas; ucb::UniCodebase=UNICODEBASE, check_exitcode::Bool=true, save_codebase_mode::Symbol=:no) = trans([x]; ucb=ucb, save_codebase_mode=save_codebase_mode)
...
###############################################################################
# HELPERS
function ucm(ss::AbstractVector{<:AbstractString}; ucb::UniCodebase=UNICODEBASE, suppress_error_nothing_to_show::Bool=false)
    stanza = StanzaUCM(ss)
    transresult = trans([stanza]; ucb=ucb, suppress_error_nothing_to_show=suppress_error_nothing_to_show)
    return transresult.transouts
end